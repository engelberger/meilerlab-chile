# PowerShell script to download weights for ESM, MIF-ST, and AlphaFold2
...
function Download-AlphaFold2-Weights {
    Write-Host "Downloading AlphaFold2 weights..."
    $af2Dir = Join-Path $PWD "alphafold2"
    New-Item -ItemType Directory -Force -Path $af2Dir
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Host "Error: AlphaFold2 weights download failed"
        exit 1
    }
    
    Write-Host "AlphaFold2 weights have been successfully downloaded to $af2Dir"
}
...
try {
    ...
    if (-not (Get-Command docker -ErrorAction SilentlyContinue)) {
        Write-Host "Error: Docker is not installed or not in PATH. Please install Docker and try again."
        exit 1
    }
    ...
    Write-Host "AlphaFold2 weights: $PWD\alphafold2"
}
catch {
